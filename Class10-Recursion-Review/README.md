![cf](http://i.imgur.com/7v5ASc8.png) Class 10: Recursion and Stacks & Queues
=====================================

## Learning Objectives
1. Students will know how to implement recursion
2. Students wil understand what happens when recursion does not stop
3. Students will be able to implement a Stack
4. Students will be able to implement a Queue
...
- If the method above calls -1, the recusion never ends. it will go to `-1! = -1 * -2! etc...`
- A way to prevent this is to add a check that says `if(n <= 0) then return 1`. (issues with this is the misleading 1 that is being sent. may be better to just throw an exception)
- Factorial method can grow very very quickly. Be careful how you are using it.
